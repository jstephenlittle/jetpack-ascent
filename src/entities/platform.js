...
export function createCheckpoint(k, x, y, width = 200) {
    ...
    const platform = k.add([
        ...
        k.area(),
        k.body({ isStatic: true }),
        k.color(220, 180, 50),  // Start yellow (inactive)
        k.outline(2, k.rgb(180, 140, 30)),
        "platform",
        "checkpoint",
        {
    ...
    platform.onCollide("player", (player) => {
        if (!platform.activated) {
            ...
            // Save checkpoint position (center of platform, above it)
            player.checkpointPos = k.vec2(platform.pos.x + width / 2, platform.pos.y - 50);
            console.log(`[CHECKPOINT] Checkpoint saved at (${player.checkpointPos.x}, ${player.checkpointPos.y})`);

            // Refill fuel to max
            player.fuel = player.maxFuel;
            console.log(`[CHECKPOINT] Fuel refilled to ${player.fuel}`);

            // Change to green (activated)
            platform.color = k.rgb(80, 220, 100);
            platform.outline = { width: 2, color: k.rgb(50, 160, 70) };
            flagBanner.color = k.rgb(80, 220, 100);
            flagBanner.outline = { width: 1, color: k.rgb(50, 160, 70) };
        }
    });
    ...
    platform.onUpdate(() => {
        ...
        if (platform.activated) {
            // Green glow when activated
            const glow = 0.85 + Math.sin(platform.pulseTime * 3) * 0.15;
            platform.color = k.rgb(80 * glow, 220 * glow, 100 * glow);
            ...
            lineDecor.forEach((line, i) => {
                ...
                line.color = k.rgb(80 * lineGlow, 220 * lineGlow, 100 * lineGlow);
            });
        } else {
            // Yellow pulse when not yet activated
            const pulse = 0.7 + Math.sin(platform.pulseTime * 4) * 0.3;
            platform.color = k.rgb(220 * pulse, 180 * pulse, 50);
